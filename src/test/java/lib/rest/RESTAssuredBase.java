...
public class RESTAssuredBase extends PreAndTest{
	...
	public static String getAccessToken() {
		...
		String payload = "{\r\n" + 
				"  \"email\": \"superadmin@crmfarm.com\",\r\n" + 
				"  \"password\": \"123456\"\r\n" + 
				"}";
		
		request.header("Content-Type","application/json");
		
		Response responseFromGenerateToken = request.body(payload).post("/Auth");
		
		responseFromGenerateToken.prettyPrint();

		 String access_token = responseFromGenerateToken.jsonPath().get("token").toString();
	...
	public static Response get(String URL, String key, String Value) {
		return setLogs()
				.when()
	...
	public static String authenticate(Response response) {
		if(response.getContentType().contains("json")) {
			JsonPath jsonPath = response.getBody().jsonPath();
			return (String) jsonPath.get("token");	
